Feature: As a user I should be able to create an Event
    ...
    @OPC-673 @WIP
    Scenario: The user should be able to create an event
      Given the user is on the login page
      Then the user enters by using "helpdesk101@cybertekschool.com" and "UserUser"
      Then the user should be on the home page
      When the user should click the events button
      And the user writes "Product Demo Presentation" as an event name
      Then the user clicks send button
      Then the user sees "Product Demo Presentation" as an event name on the page
